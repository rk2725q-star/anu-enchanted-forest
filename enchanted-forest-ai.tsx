...
import { chatWithAnu, generateAnuVoice, generateAnuImage } from './src/services/geminiService';
...
export default function App() {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const aiData = await chatWithAnu(currentInput, messages, memories);
  ...
  return (
    <div className="h-screen w-full flex flex-col relative">
      ...
      <main ref={scrollRef} className="flex-1 overflow-y-auto p-4 flex flex-col gap-2 bg-black/20 backdrop-blur-sm">
      ...
      <footer className="wood-panel p-4 flex items-center gap-3">
        ...
        <div className="flex-1 relative">
          <input
            ...
            className="w-full bg-black/40 border border-glow/30 rounded-full px-4 py-2 focus:outline-none focus:border-glow transition-all"
          />
        </div>
        {inputText.trim() ? (
